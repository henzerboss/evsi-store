...
//
// AI coach route for the QuitNic AI: Quit Vaping mobile app.
...
const QUIT_VAPING_POLICY = [
  'You are serving QuitNic AI: Quit Vaping.',
  'Stay focused on quitting nicotine vaping, cravings, triggers, slips and supportive habit change.',
  'Account for disposables, pod systems, refillable devices, frequent micro-dosing and hand-to-mouth routines.',
  'Never copy smoking-only carbon-monoxide, cancer-risk or cigarette-pack timelines into vaping advice.',
  'Do not diagnose, prescribe or give medication doses. Encourage qualified medical or crisis support when appropriate.',
  'Be non-judgmental: a slip is useful information and never erases prior progress.',
].join(' ');

function withQuitVapingPolicy(payload: unknown): unknown {
  if (typeof payload !== 'object' || payload === null) return payload;
  const root = payload as Record<string, unknown>;
  if (!Array.isArray(root.contents)) return payload;
  return {
    ...root,
    contents: [
      { role: 'user', parts: [{ text: QUIT_VAPING_POLICY }] },
...
export async function POST(req: Request) {
  ...
  const apiKey =
    process.env.GEMINI_API_KEY_QUITVAPE ??
    process.env.GEMINI_API_KEY_QUITSMOKE ??
    process.env.GEMINI_API_KEY;
  if (!apiKey) {
    return json(
      { error: 'GEMINI_API_KEY_QUITVAPE, GEMINI_API_KEY_QUITSMOKE or GEMINI_API_KEY is required' },
      500,
    );
  }
  ...
  payload = withQuitVapingPolicy(payload);
