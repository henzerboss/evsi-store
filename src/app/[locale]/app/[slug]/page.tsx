...
// КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: Обновляем сигнатуру для generateMetadata
...
export default async function AppPage({ params }: { params: Promise<{ locale: string, slug: string }> }) {
  ...
  const title = getLocalizedValue(app, 'title', locale) || app.slug;
  const description = getLocalizedValue(app, 'description', locale) || 'No description available.';
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="flex flex-col items-center gap-8 md:flex-row md:items-start">
        <Image
          ...
          width={160}
          height={160}
          className="rounded-[44px] border shadow-md flex-shrink-0"
        />
        <div className="flex flex-col items-center text-center md:items-start md:text-left">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3 md:justify-start">
            ...
            <Button asChild variant="secondary"><InternalLink href={`/app/${slug}/privacy`}><BookText /> Privacy Policy</InternalLink></Button>
          </div>
        </div>
      </div>
