...
export default function TgAppPage() {
      ...
      for (let i = 0; i < attempts; i++) {
          try {
              ...
              if (genRes.status === 400) {
                  ...
                  if (j?.code === 'ORDER_NOT_READY') { await new Promise(res => setTimeout(res, 2000)); continue; }
  ...
  const handleAiFix = async () => {
      ...
      try {
          ...
          } else { setIsAiLoading(false); alert('Ошибка создания счета'); }
      } catch { setIsAiLoading(false); alert('Ошибка соединения'); }
  };
