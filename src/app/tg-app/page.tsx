    ...
    return (
      <div className="mb-4 relative">
        <div className="flex justify-between mb-1">
          ...
          <span className={`text-xs ${isOverLimit ? 'text-red-500' : 'text-gray-300'}`}>{currentLength}/{limit}</span>
        </div>
    ...
    <div className="space-y-4">
      {activeTab === 'RESUME' && setResumeMode && (hasCorrectedVersion || (aiChanges && aiChanges.length > 0)) && (
        <div className="flex bg-gray-200 p-1 rounded-lg mb-4">
          <button onClick={() => setResumeMode('ORIGINAL')} className={`flex-1 py-2 text-sm font-bold rounded-md transition ${resumeMode === 'ORIGINAL' ? 'bg-white text-black shadow-sm' : 'text-gray-500'}`}>
            Оригинал
          </button>
          <button onClick={() => setResumeMode('CORRECTED')} className={`flex-1 py-2 text-sm font-bold rounded-md transition ${resumeMode === 'CORRECTED' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-500'}`}>
            Исправленная ✨
          </button>
        </div>
      )}
            ...
            {list.map((ch) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-xs font-bold bg-yellow-100 text-yellow-700 px-2 py-1 rounded-full">⭐️ {ch.priceStars}</div>
                </div>
              );
            })}
          </div>
...
export default function TgAppPage() {
  ...
  const totalPrice =
    activeTab === 'RANDOM_COFFEE' ? 100 : channels.filter((c) => selectedIds.includes(c.id)).reduce((sum, c) => sum + c.priceStars, 0);
    ...
    <div className="min-h-screen font-sans bg-[#f3f4f6] text-gray-900 pb-24">
      ...
      <div className="p-4 max-w-lg mx-auto">
        ...
        {step === 3 && activeTab !== 'RANDOM_COFFEE' && (
          <>
            <h2 className="text-xl font-bold mb-4 px-1">Каналы</h2>
          ...
          <div className="max-w-lg mx-auto flex items-center gap-4">
            {(step === 3 || step === 4) && activeTab !== 'RANDOM_COFFEE' && (
              <div className="flex-1">
                ...
                <div className="text-lg font-bold">⭐️ {totalPrice}</div>
              </div>
            )}
            ...
            {activeTab === 'RANDOM_COFFEE' && step === 4 && (
              <div className="flex-1">
                ...
                <div className="text-lg font-bold">⭐️ 100</div>
              </div>
            )}
